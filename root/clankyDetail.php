<?php
	...
	$Detail = new App\Views\details\ArticlesDetailView($_GET['urlpath']);
	...
	$fullPath = "clanky/" . $_GET['urlpath'];

	if ($Detail->getType() === "report") {
		$Report = new App\Views\ReportsView(new App\Models\ReportsModel);
		$reportInfo = $Report->showReportData($Detail->getId())[0];

		$reportEventDate = $reportInfo['event_date'];
		$reportVenue = $reportInfo['venue'];
	}
?>
...
<body class="<?php if (isset($_SESSION['darkmode']))echo $_SESSION['darkmode']?>">	
	...
	<div id="detail-header" class="c-header-large" style="background-image: url(../<?php $Detail->imgUrl() ?>), url(../images/default.png)">
		<div class="c-header-large__container" >			
			...
			<span class="c-tag"><?php echo $Detail->getType() ?></span> <span id="slideToCommentSection" class="c-item-footer__comments-count c-item-footer__comments-count--dark u-invert u-c-pointer"></span>
			...
			if ($Detail->getType() === 'report') {
				echo <<< EOT
					<h3 class='u-white'>$reportEventDate</h3>
					<h4 class='u-white'>$reportVenue</h4>
				EOT;
			}
			?>
		</div>
	...
	<div class="o-wrapper">
		<div class="o-container-outer">
			<div class="o-container-inner u-pos-rel">				
				<?php $Detail->showDetail() ?>
				<div class="c-article-slider"></div>
			</div>
